...
export const AdminPanel: React.FC<AdminPanelProps> = (props) => {
  // Adicionamos 'IMPORT' aos estados possíveis da aba
  const [activeTab, setActiveTab] = useState<'ASSETS' | 'EMPLOYEES' | 'TEAMS' | 'ROLES' | 'IMPORT'>('ASSETS');
  const [isEditing, setIsEditing] = useState(false);
  const [formState, setFormState] = useState<any>({});
  
  // Estado para controlar o Modal de visualização de ativos de um funcionário
  ...
  // --- MODAL DE ATIVOS DO FUNCIONÁRIO ---
            ...
            {userAssets.length === 0 ? (
              ...
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-50 text-xs uppercase font-bold text-slate-500">
                  <tr>
                    <th className="p-3 rounded-l-lg">Identificação</th>
  ...
  const renderTable = () => {
    // Se for IMPORT, a tabela não deve ser renderizada aqui
        ...
        {data.map((item: any) => (
          <tr key={item.id} className="text-sm hover:bg-slate-50 transition-colors">
            <td className="px-6 py-4 font-bold text-slate-700">
              {activeTab === 'ASSETS' ? (
                <div>
                  ...
                  {/* Visualização: Prioriza TAG, se não tiver, mostra Serial grande */}
            ...
            <td className="px-6 py-4 text-right whitespace-nowrap">
              {/* Botão de Ver Ativos (Apenas Funcionários) */}
              {activeTab === 'EMPLOYEES' && (
                <button 
      ...
      <div className="p-8 flex-1">
        ...
        {activeTab === 'IMPORT' ? (
           ...
           <DataImporter onSuccess={() => {
              ...
              alert('Importação concluída! Atualizando lista...');
              setActiveTab('ASSETS'); 
           }}/>
        ) : !isEditing ? (
          <>
          ...
          <form onSubmit={handleSubmit} className="bg-slate-50 p-8 rounded-2xl border border-slate-200 shadow-inner space-y-6 max-w-4xl mx-auto">
            ...
            {activeTab === 'ASSETS' && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {/* Serial / IMEI (Obrigatório) */}
                <div>
                   <label className={LabelClass}>Serial / IMEI (Obrigatório)</label>
                   <input 
                      className={InputClass} 
                      placeholder="Identificador de Fábrica"
                      required 
            ...
            {activeTab === 'EMPLOYEES' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex items-center mt-6">
                  ...
                  <label className="text-sm text-slate-700 font-bold">Funcionário Ativo</label>
                </div>
              </div>
            )}
